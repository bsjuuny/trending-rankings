...
export default function MindmapPage() {
    ...
    return (
        <div className="min-h-screen bg-[#0f172a] text-white p-4 md:p-8 flex flex-col items-center overflow-x-hidden font-sans">
            <Head>
                <title>커뮤니티 트렌드 마인드맵</title>
            </Head>
            
            <header className="mb-8 mt-4 text-center w-full max-w-5xl">
                <h1 className="text-3xl md:text-5xl font-black mb-3 bg-gradient-to-r from-red-400 to-rose-600 bg-clip-text text-transparent inline-block">
                    🔥 실시간 커뮤니티 트렌드
                </h1>
            </header>
            ...
            {loading ? (
                ...
                <div className="w-full max-w-6xl flex flex-col gap-12 items-center">
                    ...
                    {/* Radial Hub and Spoke Mindmap (SVG Native Scaling) */}
                    <div className="w-full max-w-[800px] w-full bg-[#1e293b] rounded-3xl shadow-2xl border border-[#334155] p-2 md:p-8 overflow-hidden relative">
                        
                        {/* ViewBox ensures perfectly proportional scaling like an image, preventing any squishing on mobile */}
                        <svg viewBox={`0 0 ${SVG_SIZE} ${SVG_SIZE}`} className="w-full h-auto drop-shadow-xl" style={{ display: "block" }}>
                            ...
                            {words.map((w, idx) => {
                                ...
                                const theta = (ringIdx / totalInRing) * 2 * Math.PI - (Math.PI / 2); // Start from top 12 o'clock
